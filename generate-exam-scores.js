...
async function generateExamScores() {
  ...
  try {
    ...
    console.log(`📊 开始为 ${students.length} 名学生生成第一周周考成绩...`);

    let insertCount = 0;
    
    for (const student of students) {
      // 根据学生ID生成不同的成绩（模拟真实情况）
      const baseScore = 400 + (student.id % 30) * 10; // 基础分400-690
      const variance = Math.floor(Math.random() * 60) - 30; // 随机波动-30到+30
      
      const chineseScore = Math.min(150, Math.max(80, 100 + (student.id % 15) * 3 + Math.random() * 20));
      const mathScore = Math.min(150, Math.max(70, 90 + (student.id % 20) * 3 + Math.random() * 25));
      const englishScore = Math.min(150, Math.max(75, 95 + (student.id % 18) * 3 + Math.random() * 22));
      const comprehensiveScore = Math.min(300, Math.max(180, 200 + (student.id % 25) * 4 + Math.random() * 40));
